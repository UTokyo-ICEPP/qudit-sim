...
def fidelity_maximization(
    ...
    **kwargs
    ...
    if save_result_to:
        with h5py.File(f'{save_result_to}_ext.h5', 'w'):
            pass

    heff_compos = _fidelity_maximization(time_evolution, tlist, dim, save_result_to, optimizer, init, max_updates, convergence)

    # if residual_adjust > 0:
    #     heff_compos = _residual_adjustment(time_evolution, tlist, dim, save_result_to, heff_compos, residual_adjust)

    if save_result_to:
        basis = paulis.paulis(dim)
        final_fidelity = heff_fidelity(time_evolution, heff_compos, basis, tlist)

        with h5py.File(f'{save_result_to}_ext.h5', 'a') as out:
...
def _fidelity_maximization(
    ...
    # Reshape and truncate the components array accordingly
    ...
    tlist_norm = jax.device_put(tlist[1:] / time_norm, device=jax_device)
    ...
    def _loss_fn(time_evolution, params, basis_list, tlist_norm):
        fidelity = heff_fidelity(time_evolution, params[0], basis_list, tlist_norm,
                                 compos_offset=params[1], npmod=jnp)
    ...
    if optimizer == 'minuit':
        ...
        if save_result_to:
            intercept = np.concatenate(([0.], opt_params['c'][1])).reshape(basis.shape[:-2])
            ...
            with h5py.File(f'{save_result_to}_ext.h5', 'a') as out:
                ...
                out.create_dataset('intercept', data=intercept)

    if not np.all(np.isfinite(compos_norm)):
        raise ValueError('Optimized components not finite')

    ## Recover the shape and normalization of the components array before returning
...
#     compos = init * time_norm

#     if save_result_to:
#         adjustments = np.zeros((residual_adjust,) + init.shape)

#     # Adjustment is done iteratively
#     for iadj in range(residual_adjust):
#         ueff_dagger = compose_ueff(compos, basis_list, tlist_norm, phase_factor=1.)
#         target = np.matmul(time_evolution, ueff_dagger)

#         ilogtargets, _, last_valid_it = get_ilogus_and_valid_it(target)
...
#     cotangent = np.power(np.cos(theta) / np.sin(theta), supp) # operand is positive as long as end <= nsamp//2
#     denom_factor = 0.5 * nsamp / (nsamp - 1)
#     denom = (end - begin) * np.power(denom_factor, supp) + np.sum(np.power(denom_factor * np.square(cotangent), supp))
...
#         residual_adjustments = np.zeros((residual_adjust,) + tuple(np.square(dim)))
#         if residual_adjust > 0:
